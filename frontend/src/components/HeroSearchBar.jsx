...
const HeroSearchBar = ({ className = '' }) => {
          ...
          <div className="relative">
            ...
            {keyword && (
              <button
                type="button"
                onClick={resetKeyword}
                className="absolute inset-y-0 right-3 text-xs font-medium text-gray-500 hover:text-primary"
              >
                Clear
              </button>
            )}
      ...
      items: (variant = 'desktop') => (
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-base font-semibold text-gray-900">Make</p>
            </div>
            <button
              type="button"
              onClick={resetMakes}
        ...
        >
          ...
          <div className="mt-4">
            <div className="relative h-8" ref={priceSliderTrackRef}>
              ...
              />
              <div
                className="absolute top-1/2 w-4 h-4 bg-white border-2 border-primary rounded-full -translate-y-1/2"
  ...
  return (
    <>
      <div
        ref={containerRef}
        className={`${className} w-full max-w bg-white border border-gray-200 rounded-full lg:rounded-2xl flex items-center px-1 py-1 lg:px-4 lg:py-2 relative`}
      >
        <button
          type="button"
        ...
        <div className="w-full self-stretch flex-1 flex items-center justify-between gap-2 relative z-50">
          <div className="lg:hidden w-full h-full px-2">
            <input
              ...
              className="w-full h-full bg-transparent focus:outline-none border-none text-sm text-gray-900 placeholder:text-gray-500 focus:ring-0 p-0"
            />
          </div>
          <nav
            ...
            {menuItems.map((menu) => (
              ...
              >
                <button
                  ...
                  type="button"
                >
                  <span className="text-xs text-start w-full mb-2 text-gray-500">{menu.key}</span>
                  <div className='w-full flex items-center'>
                    ...
                    <ChevronDown
                      ...
                      } size-3 text-gray-400 ml-auto hidden md:inline-flex transition-transform`}
                    />
                  </div>
                </button>
              </div>
      ...
      {hoveredDropdown &&
        ...
        menuItems.find((m) => m.key === hoveredDropdown)?.items && (
          <div
            ...
            className="bg-white rounded-xl min-w-[280px] animate-in fade-in slide-in-from-top-2 duration-200 shadow-xl border border-gray-100"
          >
            {menuItems.find((m) => m.key === hoveredDropdown)?.items()}
          </div>
        )}
      ...
      <AnimatePresence>
        {isMobileFiltersOpen && (
          ...
          >
            <motion.div
              className="absolute inset-0 bg-white rounded-t-3xl pb-5 overflow-y-auto"
              initial={{ y: '100%' }}
              animate={{ y: 0 }}
              exit={{ y: '100%' }}
            >
